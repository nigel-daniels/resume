var size = 300, radius = (size/2);

var hue = d3.scale.ordinal().range(colorbrewer.Resume[6]);

var luminance = d3.scale.sqrt().domain([0, 1e6]).clamp(true).range([90, 20]);

var radialGraph = d3.select('#graph').append('svg')
			.attr('width', size)
			.attr('height', size)
			.append('g')
...
d3.json("./data/tech.json", function(error, root) {
  ...
  var center = radialGraph.append('circle')
      .attr('r', radius / 3)
  ...
  var path = radialGraph.selectAll('path')
  ...
  var text = radialGraph.selectAll('text')
      	.data(partition.nodes(root).slice(1))
    	.enter().append('text')
